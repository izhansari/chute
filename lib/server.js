...
function createLanDrop({ config, dataDir, allowPublic = false, log = console.log }) {
  ...
  const MAX_TOTAL = Number(config.maxTotalMB || 0) * 1024 * 1024; // 0 = unlimited
  const fmtGB = (b) => (b >= 1024 ** 3 ? (b / 1024 ** 3).toFixed(b % 1024 ** 3 ? 1 : 0) + ' GB' : Math.round(b / 1024 ** 2) + ' MB');
  ...
  const usedBytes = () => listItems().reduce((n, it) => n + (it.size || 0), 0);
  ...
  function handleUpload(req, res) {
    ...
    const usedAtStart = MAX_TOTAL ? usedBytes() : 0;
    if (MAX_TOTAL && Number.isFinite(declared) && usedAtStart + declared > MAX_TOTAL) return send(res, 507, { error: `the chute is full (${fmtGB(MAX_TOTAL)}). Delete something first.` });
    ...
    req.on('data', (chunk) => {
      ...
      if (MAX_TOTAL && usedAtStart + size > MAX_TOTAL) return fail(507, `the chute is full (${fmtGB(MAX_TOTAL)}). Delete something first.`);
  ...
  function serveStatic(req, res, entry) {
    fs.readFile(path.join(PUBLIC_DIR, entry.file), (err, data) => {
      ...
      if (entry.file === 'index.html') {
        // The desktop app loads "/?desktop=<platform>"; tagging <html> here means the very first paint is already transparent.
        const platform = new URL(req.url, 'https://localhost').searchParams.get('desktop');
        if (platform && /^[a-z0-9]+$/.test(platform)) data = Buffer.from(data.toString('utf8').replace('<html lang="en">', `<html lang="en" class="desktop ${platform === 'darwin' ? 'mac' : 'win'}">`));
      }
      res.writeHead(200, {
  ...
  function onRequest(req, res) {
    ...
    if (p === '/api/salt' && req.method === 'GET') {
      return send(res, 200, { salt: config.salt, iterations: config.kdfIterations, ttlHours: config.ttlHours, maxMB: config.maxMB, maxTotalMB: config.maxTotalMB || 0 });
    }
    ...
    if (p === '/api/items') {
      if (req.method === 'GET') { const items = listItems(); return send(res, 200, { items, now: Date.now(), usedBytes: items.reduce((n, it) => n + (it.size || 0), 0), maxTotalBytes: MAX_TOTAL }); }
  ...
  return {
    ...
    stats() {
      ...
      const ips = [];
      for (const [ip, t] of clients) { if (t < cutoff) clients.delete(ip); else if (ip !== '127.0.0.1' && ip !== '::1') ips.push(ip); }
      return { connectedDevices: ips.length, ips };
    },
    fingerprint() {
