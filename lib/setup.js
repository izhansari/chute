...
function makeCertWithOpenssl(keyPath, certPath) {
  ...
  execFileSync('openssl', [
    ...
    '-subj', '/CN=LAN Drop', '-addext', `subjectAltName=${sans}`,
...
function makeCertWithSelfsigned(keyPath, certPath) {
  ...
  const pems = selfsigned.generate([{ name: 'commonName', value: 'LAN Drop' }], {
...
/** Generates key.pem/cert.pem in dir. Tries openssl first, then the pure-JS generator. */
function makeCert(dir) {
  const keyPath = path.join(dir, 'key.pem');
  const certPath = path.join(dir, 'cert.pem');
  try {
    makeCertWithOpenssl(keyPath, certPath);
  } catch (e1) {
    try {
      makeCertWithSelfsigned(keyPath, certPath);
    } catch (e2) {
      const err = new Error('Could not create a TLS certificate: openssl is not available and the pure-JS generator failed.');
      err.cause = { openssl: String(e1.stderr || e1.message), selfsigned: e2.message };
      throw err;
    }
  }
